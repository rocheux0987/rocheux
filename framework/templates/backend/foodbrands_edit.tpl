         ...
         <form class="form-horizontal" id="form-primary" method="POST">
            ...
            {include file="`$smarty.const._TPL_BACKEND_DIR_`common_templates/titles.tpl" type=3 title="<img src='img/flags/`$row.icon`'> `$row.name`"}
               <div class="control-group">
                  <label class="control-label" for="name">Food brand </label>
                  <div class="controls">
                     <input type="text" class="span6 foodbrands" id="name" name="form_data[{$row.lang_code}][value]" value="{$foodbrand_lang[$row.lang_code].value}">
                  </div>
               </div>
               {/foreach}
               {include file="`$smarty.const._TPL_BACKEND_DIR_`common_templates/titles.tpl" type=3 title="General"}
                <div class="control-group">
                  ...
                  <div class="controls">
                     <input type="text" class="span6" id="position" name="extra_form_data[position]" value="{$foodbrand_data.position}">
                  </div>
               </div>
               <div class="control-group">
                    ...
                    <div class="controls">
                        ...
                        <option value="A" {if $foodbrand_data.status eq 'A'} selected="selected"{/if}>Active</option>
                        <option value="D" {if $foodbrand_data.status eq 'D'} selected="selected"{/if}>Disabled</option>
                        <option value="H" {if $foodbrand_data.status eq 'H'} selected="selected"{/if}>Hidden</option>
                        </select>
                    </div>
                </div>
            ...
            <input type="hidden" name="extra_form_data[food_brand_id]" id="lang_code" value="{$smarty.get.food_brand_id}" />
         </form>
      </div>
   </div>
   <!--/span-->
