         ...
         <form class="form-horizontal" id="form-primary" method="POST">
            ...
            {include file="`$smarty.const._TPL_BACKEND_DIR_`common_templates/titles.tpl" type=3 title="<img src='img/flags/`$row.icon`'> `$row.name`"}
               <div class="control-group">
                  <label class="control-label" for="name">Breed </label>
                  <div class="controls">
                     <input type="text" class="span6 breeds" id="name" name="form_data[{$row.lang_code}][value]" value="{$breed_lang[$row.lang_code].value}">
                  </div>
               </div>
               {/foreach}
               {include file="`$smarty.const._TPL_BACKEND_DIR_`common_templates/titles.tpl" type=3 title="General"}
                <div class="control-group">
                  ...
                  <div class="controls">
                     <input type="text" class="span6" id="position" name="extra_form_data[position]" value="{$breed_data.position}">
                  </div>
               </div>
               <div class="control-group">
                    ...
                    <div class="controls">
                        ...
                        <option value="A" {if $breed_data.status eq 'A'} selected="selected"{/if}>Active</option>
                        <option value="D" {if $breed_data.status eq 'D'} selected="selected"{/if}>Disabled</option>
                        <option value="H" {if $breed_data.status eq 'H'} selected="selected"{/if}>Hidden</option>
                        </select>
                    </div>
                </div>
            ...
            <input type="hidden" name="extra_form_data[breed_id]" id="lang_code" value="{$smarty.get.breed_id}" />
         </form>
      </div>
   </div>
   <!--/span-->
