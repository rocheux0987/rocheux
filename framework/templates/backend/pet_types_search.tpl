...
function filter_search(){
	if (document.getElementById("pet_type").value == ""){
		notification("At least one field must be filled to continue", "error", true);
		return false;	
	}
	document.location.href = "?act=list&pet_type="+document.getElementById("pet_type").value;
}
...
<div class="row-fluid">
	<div class="box span12">
        ...
        <div class="box-content" {if $smarty.get.pet_type eq ''}style="display:none"{/if}>
            <table class="table table-striped table-bordered">
              <thead>
                  <tr>
                      <th width="25%" ><label>Pet type<input class="field-search enter-trigger-search" type="text" id="pet_type" name="pet_type" value="{$smarty.get.pet_type}" /></label> </th>
                      <th width="25%"></th>
                      <th width="25%"></th>
                      <th width="25%"></th>
                  </tr>
