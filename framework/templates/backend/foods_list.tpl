...
{include btn_onclick="document.location.href='`$url_add`';" btn_size="btn-success" btn_type="btn-primary" btn_title="Add Food" file="`$smarty.const._TPL_BACKEND_DIR_`common_templates/buttons.tpl"}
</div>
...
<div class="row-fluid">            
   <div class="box span12">
      ...
      <div class="box-content">        
      	{* Paginator *}
        {capture name="pager"}
			{include file="`$smarty.const._TPL_BACKEND_DIR_`common_templates/pager.tpl" paginator=$foods_list}
		{/capture}
        ...
        <form action="?act=edit" class="form-horizontal" id="form-primary" method="POST">
         <table class="table table-striped table-bordered">
            ...
            <tbody>
            	{foreach name="results" from=$foods_list.data item=row}
               <tr>
               	  <td>{$row.pet_type_value}</td>
                  <td><input type="text" class="languages-textarea" name="foods[{$row.food_id}][value]" id="food_{$row.food_id}" value="{$row.value}" /></td>
                  <td><input type="text" name="foods[{$row.food_id}][position]" id="foods_order_{$row.pet_type_id}" value="{$row.position}" /></td>
                  <td class="center">
                  	{if $row.status eq "A"}
                  ...
                  <td>
					<a class="btn btn-info" href="{$url_edit}&food_id={$row.food_id}">
                    	<i class="halflings-icon white edit"></i>
                    </a>
                    <a class="btn btn-danger" href="{$url_delete}&food_id={$row.food_id}" onclick="return confirm('Are you sure you want to delete this item?')" title="Delete">
